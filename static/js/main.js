// static/js/main.js (FİYAT SORUNU DÜZELTİLDİ)
...
async function verileriDisaAktar() {
    const tarih = ui.tarihFiltreleyici ? utils.getLocalDateString(ui.tarihFiltreleyici.selectedDates[0]) : null;
    try {
        const { filename, blob } = await api.fetchCsvExport(tarih);
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = filename; 
        document.body.appendChild(a); a.click(); a.remove();
    } catch(e) {
        gosterMesaj("Dışa aktarma başarısız.", "danger");
    }
}
...
function setupComponents() {
    ...
    if(selectEl && typeof TomSelect !== 'undefined') {
        ui.tedarikciSecici = new TomSelect(selectEl, {
            ...
            onChange: (val) => { if(val) guncelFiyatiGetir(val); } // Tedarikçi seçilince fiyatı getir
        });
    }
}
...
async function verileriYukle() {
    ...
    // Başlangıçta fiyatı çek (Genel Tarife)
...
// FİYAT ÇEKME FONKSİYONU (GÜÇLENDİRİLDİ)
async function guncelFiyatiGetir(tedarikciId = null) {
    ...
    try {
        // 1. Önce Tarifeye Bak (En öncelikli)
        const tarife = await api.fetchTarifeFiyat(tarih);
        // Backend { fiyat: "10.00" } dönüyor
        if(tarife && tarife.fiyat) {
            input.value = parseFloat(tarife.fiyat).toFixed(2);
            return; // Tarife varsa çık
        }
    } catch(e) { console.log("Tarife yok"); }

    // 2. Tarife yoksa Tedarikçinin Son Fiyatına bak
    if(tedarikciId) {
        try {
    ...
    input.value = ''; // Manuel girişe zorla
}
...
async function sutGirdisiEkle() {
    ...
    // Fiyat kontrolü (NaN ise 0 yapma, kullanıcıyı uyar)
    ...
    try {
        ...
        ui.resetGirdiFormu();
        // Kayıttan sonra fiyatı tekrar getirmeye çalış (Varsayılanı korumak için)
        guncelFiyatiGetir(); 
        verileriYukle();
    } catch(e) {
