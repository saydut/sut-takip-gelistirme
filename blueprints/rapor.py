...
# --- YENİ ENDPOINT: 3'LÜ PASTA GRAFİK VERİSİ ---
@rapor_bp.route('/tedarikci_dagilimi_coklu')
@login_required
def get_tedarikci_dagilimi_coklu():
    """Günlük, Haftalık ve Aylık tedarikçi dağılımlarını tek seferde döndürür."""
    try:
        sirket_id = session['user']['sirket_id']
        
        # Yardımcı Fonksiyon: Belirli bir periyot için dağılımı al ve formatla
        def get_distribution(period_type):
            response = g.supabase.rpc('get_supplier_distribution', {
                'p_sirket_id': sirket_id,
                'p_period': period_type
            }).execute()
            
            data = response.data or []
            labels = [item['name'] for item in data]
            values = [float(item['litre']) for item in data]
            return {'labels': labels, 'data': values}

        return jsonify({
            'daily': get_distribution('daily'),
            'weekly': get_distribution('weekly'),
            'monthly': get_distribution('monthly')
        })

    except Exception as e:
        logger.error(f"Çoklu tedarikçi dağılımı alınırken hata: {e}", exc_info=True)
        return jsonify({"error": "Veriler alınırken bir hata oluştu."}), 500
# -----------------------------------------------------------
